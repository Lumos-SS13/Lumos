...
/obj/effect/slime_rune/rainbow_room/Crossed(atom/movable/AM, oldloc)
	...
	if(safeTurf)
		var/list/possible_dir = list()
		for(var/cho_direction in GLOB.cardinals)
			var/turf/step_turf = get_step(safeTurf, cho_direction)
			if(isfloorturf(step_turf))
				possible_dir += step_turf
		safeTurf = pick(possible_dir)
	if(!safeTurf)
