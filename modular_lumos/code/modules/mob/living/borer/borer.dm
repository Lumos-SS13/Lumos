...
/mob/living/captive_brain/say(message, bubble_type, spans = list(), sanitize = TRUE, datum/language/language = null, ignore_spam = FALSE, forced = null)
...
/mob/living/captive_brain/proc/return_control(mob/living/simple_animal/borer/B)
    if(!B || !B.controlling)
        return
    B.victim.adjustOrganLoss(ORGAN_SLOT_BRAIN, rand(5,15))
...
/mob/living/simple_animal/borer
	...
	icon = 'modular_lumos/icons/mob/animal.dmi'
	...
	response_harm_continuous = "chomps"
...
/mob/living/simple_animal/borer/verb/punish()
	...
	switch(punishment) //Hardcoding this stuff.
		if("Blindness")
			victim.eye_blind += 2
		if("Deafness")
			var/obj/item/organ/ears/v_ears = victim.getorganslot(ORGAN_SLOT_EARS)
			v_ears.deaf += 20
		if("Stun")
			victim.Stun(10)
...
/datum/action/innate/borer
	...
	icon_icon = 'modular_lumos/icons/mob/actions.dmi' //This is the file for the ACTION icon
